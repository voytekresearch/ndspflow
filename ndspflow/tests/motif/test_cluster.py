...
def test_cluster_cycles():
    ...
    # Two clusters
    labels = cluster_cycles(cycles, min_clust_score=0.5, max_clusters=11)
    assert all([lab == labels[0] for lab in labels[1:5]])
    assert all([lab == labels[5] for lab in labels[6:]])

    # No clusters
    labels = cluster_cycles(cycles, min_clust_score=1.1)
    assert np.isnan(labels)

    # Invalid param passed to KMeans
    labels = cluster_cycles(cycles, min_clust_score=0.5, min_clusters=-1, max_clusters=0)
    assert np.isnan(labels)
